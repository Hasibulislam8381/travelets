...
@section('content')
    <div class="container-fluid checkout-page">
        ...
        @guest
            {{-- Guest হলে form নেই, শুধু layout দেখাবে --}}
        @else
            <form id="bookingForm" action="{{ route('checkout.pay', $product) }}" method="POST">
                @csrf
            ...
            <div class="row g-4">
                ...
                <div class="col-lg-4">
                    <div class="summary-card">
                        ...
                        @guest
                            <a href="{{ route('user.login') }}" class="btn-book-now">
                                Login to Book
                            </a>
                        @else
                            <button type="button" class="btn-book-now" id="bookBtn" onclick="submitBooking()">
                                {{ $categoryType === 'souvenirs' ? 'Buy Now' : ($categoryType === 'skill_training' ? 'Register Now' : 'Book Now') }}
    ...
    <script>
        ...
        function updateSummary() {
            ...
            if (bookBtn) {
                bookBtn.textContent = `${getActionLabel()} - Pay ৳${Math.max(0, total).toLocaleString()}`;
            }
        }

        function addTraveler() {
            const i = travelerCount; // 0-based index
            travelerCount++;
            ...
            block.innerHTML = `
                ...
                <div class="row g-3">
                    <div class="col-md-6">
                        ...
                        <input type="text" name="travelers[${i}][name]" class="form-control-custom" placeholder="Type full name" required>
                    </div>
                    <div class="col-md-6">
                        <label class="form-label-custom">E-mail<span style="color:var(--orange)">*</span></label>
                        <input type="email" name="travelers[${i}][email]" class="form-control-custom" placeholder="Type email" required>
                    </div>
                    <div class="col-md-6">
                        <label class="form-label-custom">Phone Number<span style="color:var(--orange)">*</span></label>
                        <input type="tel" name="travelers[${i}][phone]" class="form-control-custom" placeholder="Type phone number" required>
                    </div>
                    <div class="col-md-6">
                        <label class="form-label-custom">Date of Birth<span style="color:var(--orange)">*</span></label>
                        <input type="text" name="travelers[${i}][dob]" class="form-control-custom" placeholder="mm/dd/yyyy">
                    </div>
                    ${categoryType !== 'skill_training' ? `
                                        <div class="col-md-6">
                                            <label class="form-label-custom">Adult/Baby<span style="color:var(--orange)">*</span></label>
                                            <div class="select-wrap">
                                                <select name="travelers[${i}][type]" class="form-select-custom">
                                        ...
                                        <div class="col-md-6">
                                            ...
                                            <input type="text" name="travelers[${i}][nid]" class="form-control-custom" placeholder="ID number">
                                        </div>` : ''}
                    <div class="col-12">
                        <label class="form-label-custom">Full Address<span style="color:var(--orange)">*</span></label>
                        <input type="text" name="travelers[${i}][address]" class="form-control-custom" placeholder="Type full address">
                    </div>
                    <div class="col-12">
                        <label class="form-label-custom">Instruction</label>
                        <textarea name="travelers[${i}][instruction]" class="form-control-custom" rows="3" placeholder="Any special instructions" style="resize:none;"></textarea>
                    </div>
                </div>
            `;
        ...
        function applyPromo() {
            ...
            if (code === 'VROMON10') {
                discountAmount = basePrice * 0.10 * document.querySelectorAll('.traveler-block').length;
                msg.style.color = '#2d8a4e';
                msg.textContent = '✓ 10% discount applied!';
                row.style.display = 'flex';
                document.getElementById('discount-val').textContent = '-৳' + discountAmount.toLocaleString();
            } else if (code === '') {
                msg.textContent = '';
            } else {
                discountAmount = 0;
        ...
        function submitBooking() {
            ...
            let valid = true;
            document.querySelectorAll('.traveler-block input[required]').forEach(input => {
                if (!input.value.trim()) {
